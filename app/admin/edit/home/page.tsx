        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
            ...
            <div>
              ...
              <p style={{ color: 'var(--muted-2)' }}>Update the images and text that appear on the main landing page.</p>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: '32px', marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
              ...
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700 }}>Primary Hero Image</h3>
            </div>
            
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '32px' }}>
              {/* Current Image 1 */}
              <div>
                <label className="form-label" style={{ fontWeight: 600, display: 'block', marginBottom: '12px' }}>Current Primary Image</label>
                <div style={{ position: 'relative', width: '100%', height: '240px', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--border)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                  <Image src="/assets/images/1_bed_new.png" alt="Current Hero 1" fill style={{ objectFit: 'cover' }} />
                </div>
              </div>

              {/* Upload New Image 1 */}
              <div>
                <label className="form-label" style={{ fontWeight: 600, display: 'block', marginBottom: '12px' }}>Upload New Primary Image</label>
                <div 
          ...
          {/* Hero Image 2 Section */}
          <div className="card" style={{ padding: '32px', marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
              <ImageIcon size={24} color="var(--accent-2)" />
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700 }}>Secondary Image</h3>
            </div>
            
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '32px' }}>
              {/* Current Image 2 */}
              <div>
                <label className="form-label" style={{ fontWeight: 600, display: 'block', marginBottom: '12px' }}>Current Secondary Image</label>
                <div style={{ position: 'relative', width: '100%', height: '240px', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--border)', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                  <Image src="/assets/images/2_bed_new.png" alt="Current Hero 2" fill style={{ objectFit: 'cover' }} />
                </div>
              </div>

              {/* Upload New Image 2 */}
              <div>
                <label className="form-label" style={{ fontWeight: 600, display: 'block', marginBottom: '12px' }}>Upload New Secondary Image</label>
                <div 
                  style={{ width: '100%', height: '240px', border: '2px dashed var(--border-h)', borderRadius: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,0.4)', cursor: 'pointer', transition: 'all 0.2s' }} 
                  onMouseOver={(e) => { e.currentTarget.style.borderColor = 'var(--accent-2)'; e.currentTarget.style.backgroundColor = 'rgba(192,112,64,0.03)' }} 
                  onMouseOut={(e) => { e.currentTarget.style.borderColor = 'var(--border-h)'; e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.4)' }}
                >
                  <div style={{ width: '56px', height: '56px', borderRadius: '50%', backgroundColor: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px', color: 'var(--accent-2)' }}>
                    <Upload size={24} />
                  </div>
                  <span style={{ fontWeight: 600, color: 'var(--text)' }}>Click to browse or drag image here</span>
                  <span style={{ fontSize: '0.8rem', color: 'var(--muted-2)', marginTop: '8px' }}>Supported formats: JPG, PNG, WEBP (Max 5MB)</span>
